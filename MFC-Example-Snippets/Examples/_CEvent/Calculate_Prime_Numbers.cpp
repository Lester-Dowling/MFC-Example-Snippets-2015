...
IMPLEMENT_CRUNTIMECLASS_SUPPORT_DYNAMIC_CREATE(Examples::_CEvent::Calculate_Prime_Numbers)
...
namespace Examples {
	namespace _CEvent {
		...
		Calculate_Prime_Numbers::Calculate_Prime_Numbers()
		...
		void Calculate_Prime_Numbers::run()
		{
		...
		BOOL Calculate_Prime_Numbers::IsPrime(INT ThisPrime)
		{
			if (ThisPrime < 2)
				return FALSE;
		...
		INT Calculate_Prime_Numbers::NextPrime(INT ThisPrime)
		{
			while (TRUE)
			{
		...
		UINT Calculate_Prime_Numbers::PrimeCalcProc(LPVOID lpParameter)
		{
